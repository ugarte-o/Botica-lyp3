<?php
//based on HTMLtoOpenXML
class mwmod_mw_html_toopenxml {
	...
	public static function getInstance() {
		if (is_null(self::$_instance)){
			self::$_instance = new mwmod_mw_html_toopenxml();
		}
	...
	public function fromHTML($htmlCode) {
		$start = 0;
		$properties = array();
		$openxml = mwmod_mw_html_toopenxml_htmlcleaner::getInstance()->cleanUpHTML($htmlCode);
		$openxml = $this->getOpenXML($openxml);
		$openxml = $this->processBreaks($openxml);
		$openxml = $this->processListStyle($openxml);
		$openxml = mwmod_mw_html_toopenxml_processproperties::getInstance()->processPropertiesStyle($openxml, $start, $properties);
