<?php
//TOTO revisar crop
class mwmod_mw_users_ui_user_img extends mwmod_mw_users_ui_user_abs{
	...
	function do_exec_page_in(){
		...
		/*
		if($imgs=$user->get_imgs()){
			$imgscontainer=new mwmod_mw_html_elem();
			foreach($imgs as $img){
				if($imgelem=$img->get_img_elem()){
					$imgscontainer->add_cont($imgelem);	
				}
			}
			$panel->cont_elem->add_cont($imgscontainer);
		...
		*/
		...
		//$jsin="var crophelper=new cropper($('#crop-avatar'),".$params->get_as_js_val()."); crophelper.initPreview(); ";
