<?php
/**
 * Users2 - Change User Password (Admin)
 * 
 * Admin interface for changing user password.
 * Does NOT require current password (admin override).
 * Uses modern JS inputs pattern.
 */
class mwmod_mw_users2_ui_user_pass extends mwmod_mw_users2_ui_user_abs {
    ...
    /**
     * Execute page: show change password form
     */
    public function do_exec_page_in() {
        ...
        if ($inputMan->is_req_input_ok()) {
            if ($nd = $inputMan->get_value_by_dot_cod_as_list("pass")) {
                $dm->saveAdminChangePass($nd, $user, $msgs);
            }
        }
        ...
        // Password inputs - NO current password required (admin mode)
        $dm->addAdminChangePassInputs($mainGr, $user);
