<?php
class mwmod_mw_modulesman_ui_fileinfo extends mwmod_mw_modulesman_ui_abs{
	var $dirman;
	var $moduleinfoman;
	var $infomandata=array();
	function __construct($cod,$maininterface){
		$this->initui($cod,$maininterface);
		$this->set_def_title("File info");
	...
	function select_infoman_from_input($input){
		...
		$this->dirman=$dirman;
		$moduleinfoman=false;
		$moduleinfoman=$this->dirman->create_file_info_man_for_full_path($path);	
		if($moduleinfoman){
	...
	function do_exec_page_in(){
		...
		$i=$gr->add_item(new mwmod_mw_datafield_input("path","File"));
