<?php
class mwmod_mw_ui_debug_db_db extends mwmod_mw_ui_sub_uiabs{
	function __construct($cod,$parent){
		$this->init_as_subinterface($cod,$parent);
		$this->set_def_title("Base de datos");
		$this->subinterface_def_code="tests";
		
	}
	function add_2_sub_interface_mnu($mnu){
		if($subs=$this->get_subinterfaces_by_code("tests",true)){
	...
	function load_all_subinterfases(){
		...
		$si=$this->add_new_subinterface(new mwmod_mw_ui_debug_db_tests("tests",$this));
	}
	/*
